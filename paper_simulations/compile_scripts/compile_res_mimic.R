...
load("mimic_simulation_results_lowdim_test_n250_rho0_M200_error06_gwas.Rdata")
...
for (tau_id in names(all_results)) {
  ...
  # Process GWAS QR results
  res_trad <- apply(all_results[[tau_id]]$GWAS_qr, 1, compile_metrics, true_vars_indices = true_variables)
  results_list[[length(results_list) + 1]] <- bind_rows(res_trad, .id = "Replicate") %>%
    mutate(Tau = tau_val, Model = "GWAS", Prior = "None")
...
final_results_df <- bind_rows(results_list) %>%
  mutate(Replicate = as.integer(Replicate),
         Prior = factor(Prior, levels = c("None", "S1", "S2", "S3", "S4" )))
...
plot_df <- final_results_df %>%
  filter(Prior %in% c("None", "S1", "S2", "S3", "S4")) %>%
...
f1_boxplot_grayscale <- ggplot(plot_df, aes(x = Prior, y = F1, fill = Model)) + # Keep fill for box color
  ...
  scale_shape_manual(values = c("Trad-QR" = 24, "GWAS" = 23, "Prior-QR" = 22, "KIQR" = 21)) + # Filled shapes
  scale_fill_manual(values = c("Trad-QR" = "#F8766D", "GWAS" = "orange", "Prior-QR" = "#00BA38", "KIQR" = "#619CFF")) +
  labs(
...
ggsave("F1_mimic_n250p1500_error06_rho0_bwc.png", plot = f1_boxplot_grayscale, width = 12, height = 7, dpi = 300)
