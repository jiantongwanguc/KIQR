...
true_beta_median <- true_beta
true_beta_median[1] <- 0
...
Y_data <- t(replicate(sim_params$k, {
  # Generate a standard error term
  error_term <- rnorm(sim_params$n, sd = sim_params$sigma)
  # Make the error's scale dependent on X_1
  heteroscedastic_error <- error_term * (1 + X_data[, 1])
  # Generate Y using the heteroscedastic error
  as.vector(X_data %*% true_beta_median + heteroscedastic_error)
}))
...
save(
  ...
  sim_params,
  true_beta,
  file = "simulation_results_final_n200p1500_X1e_k200.Rdata"
)
cat("Results saved.\n")
